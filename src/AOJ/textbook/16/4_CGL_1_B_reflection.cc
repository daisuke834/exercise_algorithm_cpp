...
#include "4_CGL_1_B_reflection.h"
...
void CallReflection(std::istream& input_stream) noexcept {
  ...
  for (int32_t i = 0; i < number_of_queries; ++i) {
    double x, y;
    input_stream >> x >> y;
    const Point2D reflected_point = line.ReflectPoint(Point2D(x, y));
    std::cout << std::setprecision(20) << reflected_point.X() << " " << reflected_point.Y() << std::endl;
  }
}
