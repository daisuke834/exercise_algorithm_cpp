...
#include "9_CGL_7_D_cross_points_of_circle_and_line.h"
...
void CallCrossPointsOfCircleAndLine(std::istream& input_stream) {
  ...
  const Circle circle(center_x, center_y, center_radius);
  ...
  for (int32_t i = 0; i < number_of_queries; ++i) {
    ...
    const Line line(Point2D(p1_x, p1_y), Point2D(p2_x, p2_y));
    const std::pair<Point2D, Point2D> cross_points = circle.GetCrossPointsWithLine(line);
