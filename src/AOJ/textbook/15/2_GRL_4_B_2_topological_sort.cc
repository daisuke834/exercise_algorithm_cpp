...
#include "2_GRL_4_B_2_topological_sort.h"
...
void TopologicalSort::GenerateSortedList() {
  try {
    for (int32_t i = 0; i < kMaxLoopCount; ++i) {
      ...
      if (DoesNotExist(target_vertex_index)) {
        if (NoMoreUnexploredIndependentVertex()) {
          break;
        } else {
          target_vertex_index = PickUnexploredIndependentVertex();
        }
      }
  ...
  if (AllVerticesAreNotCompleted()) {
    std::cerr << "ERROR: ExploreAllVertices(): All vertexes are not discovered yet." << std::endl;
    throw 1;
  }
}
  ...
  try {
    for (int32_t i = 0; i < kMaxLoopCount; ++i) {
      ...
      if (DoesNotExist(index_tail_vertex)) {
        ...
        vertices_[index_tail_vertex].DecrementRemainingIndegree();
        if (vertices_[index_tail_vertex].RemainingIndegreeIsZero()) {
          CompleteOneVertex(index_tail_vertex);
        }
      }
    }
  } catch (...) {
...
void TopologicalSort::CompleteOneVertex(const int32_t vertex_index) {
  ...
  try {
    ...
    sorted_.push_back(vertex_index);
  } catch (...) {
...
bool TopologicalSort::NoMoreUnexploredIndependentVertex() const {
  const int32_t unexplored_vertex_index = FindUnexploredIndependentVertex();
  const bool no_more = DoesNotExist(unexplored_vertex_index);
  return no_more;
}

int32_t TopologicalSort::FindUnexploredIndependentVertex() const {
  int32_t unexplored_vertex_index = kDoesNotExist;
  try {
    for (int32_t vertex_index = 0; vertex_index < number_of_vertices_; ++vertex_index) {
      const bool is_unexplored_independent_vertex = vertices_[vertex_index].GetStatus() == VertexStatus::kInit &&
                                                    vertices_[vertex_index].RemainingIndegreeIsZero();
      if (is_unexplored_independent_vertex) {
...
int32_t TopologicalSort::PickUnexploredIndependentVertex() {
  int32_t unexplored_vertex_index;
  try {
    unexplored_vertex_index = FindUnexploredIndependentVertex();
    CompleteOneVertex(unexplored_vertex_index);
  } catch (...) {
    std::cerr << "ERROR: PickUnexploredIndependentVertex()" << std::endl;
    throw;
  }
  return unexplored_vertex_index;
}
...
void TopologicalSort::MapDirection(const int32_t vertex_index_from, const int32_t vertex_index_to) {
  if (IsInvalidVertexIndex(vertex_index_from)) {
    ...
    try {
      ...
      vertices_[vertex_index_to].IncrementRemainingIndegree();
    } catch (...) {
...
void TopologicalSort::Print() {
  try {
    for (std::vector<int32_t>::iterator itr = sorted_.begin(); itr != sorted_.end(); ++itr) {
...
// *************************************
Vertex::Vertex() noexcept : status_(VertexStatus::kInit), next_check_(0), remaining_indegree_(0) {}

Vertex::~Vertex() noexcept {}

void Vertex::Completed() {
  if (status_ != VertexStatus::kInit) {
    std::cerr << "ERROR: Vertex::Completed(): Vertex is not checked yet." << std::endl;
    throw 1;
  }
...
void Vertex::IncrementRemainingIndegree() {
  if (remaining_indegree_ >= kMaxNumberOfVertices) {
    std::cerr << "ERROR: Vertex::IncrementRemainingIndegree(): remaining_indegree_ is ." << remaining_indegree_
              << std::endl;
    throw 1;
  }
  ++remaining_indegree_;
...
void Vertex::DecrementRemainingIndegree() {
  if (remaining_indegree_ <= 0) {
    std::cerr << "ERROR: Vertex::DecrementRemainingIndegree(): remaining_indegree_ is ." << remaining_indegree_
              << std::endl;
    throw 1;
  }
  --remaining_indegree_;
...
bool Vertex::RemainingIndegreeIsZero() const noexcept { return (remaining_indegree_ == 0); }
