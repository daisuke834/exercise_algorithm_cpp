 ...
 * @brief 最大公約数(Greatest Common Divisor, GCD)。ユークリッドの互除法による高速化。計算量はO(log(min(a,b)))
 */

#include "2_ALDS1_1_B_2_GCD_Euclidean.h"
...
int32_t CalculateGreatestCommonDivisor(const int32_t a, const int32_t b) {
  ...
  if ((a < 0) || (a > kMaxInputValue)) {
    ...
    const int32_t new_b = std::max(a, b) % new_a;  // Euclidean Algorithm
