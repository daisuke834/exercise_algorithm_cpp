 ...
 * @brief Part9-2. 二分探索木(Binary Search Tree)、挿入(Insert)。
...
#include "src/AOJ/textbook/09/2_ALDS1_8_A_binary_search_tree_1.h"

namespace ALDS1_8_A {

class Test_ALDS1_8_A : public ::testing::Test {
 protected:
...
TEST_F(Test_ALDS1_8_A, Node_initial) {
  Node cnode;
...
TEST_F(Test_ALDS1_8_A, Node_Activate) {
  Node cnode;
  ASSERT_FALSE(cnode.IsValid());
  cnode.Activate();
...
TEST_F(Test_ALDS1_8_A, Node_SetKey) {
...
TEST_F(Test_ALDS1_8_A, Node_SetParent) {
...
TEST_F(Test_ALDS1_8_A, Node_SetChildLeft) {
  const int32_t kId = 12;
  Node cnode;
...
TEST_F(Test_ALDS1_8_A, Node_SetChildRight) {
...
TEST_F(Test_ALDS1_8_A, Node_GetNodeType) {
  Node cnode;
  ASSERT_ANY_THROW(cnode.GetNodeType());
  cnode.Activate();
  ASSERT_ANY_THROW(cnode.GetNodeType());
...
TEST_F(Test_ALDS1_8_A, NodeConstructors) {
  Node node_invalid;
...
TEST_F(Test_ALDS1_8_A, CallBinarySearchTree_1) {
